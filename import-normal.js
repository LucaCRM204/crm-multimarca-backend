...
async function importLeads() {
  ...
  for (const lead of leads) {
    try {
      ...
      // Usar fuentes normales (sin -200)
      const fuente = lead.platform === 'fb' ? 'meta' : 'instagram';
      
      const modelo = lead.modelo === 'tera' ? 'Tera' : 'Polo';
      
      const notas = `Origen: CM\nPlataforma: ${lead.platform === 'fb' ? 'Facebook' : 'Instagram'}\nInfo adicional: ${lead.anticipo}`;
      ...
      await pool.execute(
        `INSERT INTO leads (nombre, telefono, modelo, formaPago, estado, fuente, notas, assigned_to, created_at) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [lead.nombre, telefono, modelo, 'Consultar', 'nuevo', fuente, notas, assigned_to]
      );
